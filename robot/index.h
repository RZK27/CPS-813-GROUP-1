...
function handleKeyDown(event) {
  if (event.key in keyCommandMap) {
    event.preventDefault(); // Prevent the default scrolling behavior
    send_command(keyCommandMap[event.key]);
    document.getElementById(keyCommandMap[event.key]).style.backgroundImage = 
      "url('https://esp32io.com/images/tutorial/" + img_name_lookup[keyCommandMap[event.key]] + "_active.png')";
  }
}
...
function handleKeyUp(event) {
  if (event.key in keyCommandMap) {
    ...
    document.getElementById(keyCommandMap[event.key]).style.backgroundImage = 
      "url('https://esp32io.com/images/tutorial/" + img_name_lookup[keyCommandMap[event.key]] + "_inactive.png')";
  }
}
